...
class AirflowClient:
    def __init__(
        self, base_url=None, username=None, password=None, password_file=None
    ):
        self.base_url = (
            base_url
            or os.environ.get("AIRFLOW_URL", "http://airflow-api-server:8080")
        ).rstrip("/")
    ...
    def dag_tasks(self, dag_id):
        return self.request(
            "GET", "/api/v2/dags/%s/tasks?limit=200" % self._quote(dag_id)
        )
    ...
    def reparse(self, dag_id):
        ...
        return self.request(
            "PUT", "/api/v2/parseDagFile/%s" % self._quote(token)
        )

    def update_dag(self, dag_id, paused):
        return self.request(
            "PATCH",
            "/api/v2/dags/%s" % self._quote(dag_id),
            {"is_paused": paused},
        )
    ...
    def trigger(self, dag_id, conf=None):
        return self.request(
            "POST",
            "/api/v2/dags/%s/dagRuns" % self._quote(dag_id),
            {
                "conf": conf or {},
    ...
    def request(self, method, path, payload=None):
        ...
        try:
            ...
            raise AirflowError(
                "Airflow returned HTTP %s: %s" % (error.code, detail)
            ) from error
    ...
    def _token(self):
        ...
        try:
            with urllib.request.urlopen(request, timeout=10) as response:
                token = json.loads(response.read().decode()).get(
                    "access_token"
                )
        except (OSError, ValueError) as error:
            raise AirflowError(
                "Airflow authentication failed: %s" % error
            ) from error
        if not token:
            raise AirflowError(
                "Airflow authentication returned no access token"
            )
        return token
    ...
    def _password_from_file(self):
        try:
            ...
            raise AirflowError(
                "cannot read Airflow password for %s: %s"
                % (self.username, error)
            ) from error
